...
@Slf4j
@Service
public class PineconeVectorStoreServiceImpl implements PineconeVectorStoreService {
    ...
    public void storeStockData(StockDataMessage message) {
        try {
            // 检查VectorStore是否可用
            if (vectorStore == null) {
                log.warn("VectorStore未配置，跳过存储股票数据: stockCode={}, dataType={}",
                        message.getStockCode(), message.getDataType());
                return;
            }
    ...
    public void batchStoreStockData(List<StockDataMessage> messages) {
        try {
            // 检查VectorStore是否可用
            if (vectorStore == null) {
                log.warn("VectorStore未配置，跳过批量存储股票数据: count={}", messages.size());
                return;
            }
